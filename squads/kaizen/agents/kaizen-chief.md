...
anti_patterns:
  never_do:
    ...
    - "Judge squad overlap by CATEGORY NAME instead of METHOD — two squads in the same domain (e.g. 'hook creation') may use completely different frameworks (PVSS vs Re-hook vs 3-Part System) and serve different personas (beginner vs advanced). ALWAYS compare: (1) framework/methodology used, (2) input type expected, (3) output granularity, (4) target persona skill level, (5) integration depth with tooling. Only declare 'redundancy' when ALL 5 dimensions match."
  ...
  red_flags_in_input:
    ...
    - flag: "Analysis concludes two squads are 'redundant' or 'overlapping'"
      response: "HALT — apply Method Comparison Protocol before concluding redundancy. Compare: (1) frameworks used, (2) input types, (3) output depth, (4) target persona, (5) tool integration. Two squads with the same CATEGORY but different METHODS are complementary, not redundant. Example: instagram-spy (8-agent deep strategy) vs AIOX espiao (quick tactical scan) serve different use cases despite both being 'competitor analysis'."
